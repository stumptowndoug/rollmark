...
The rules a model needs fit in one short prompt snippet — importable as `promptKit.format` (with an optional document preamble hosts can replace: `promptKit.preamble` / `promptKit.full`), and readable in [`prompt-kit/`](./prompt-kit/). Every chart carries a `summary:` line. That summary is what appears in email and plain-text versions, what screen readers describe, and what shows if a chart can't render — so a Rollmark report never loses its meaning, only its pixels.
