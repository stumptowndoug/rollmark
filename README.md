...
```ts
import { renderRollmark, compileToECharts } from "rollmark";
...
for (const block of blocks) {
  if (block.type === "chart" && block.spec) {
    const option = compileToECharts(block.spec); // plain ECharts option object
    // mount into the matching placeholder with your echarts instance
  }
...
| `src/` | Parser plugin, validator, fallback rendering, ECharts compiler |
